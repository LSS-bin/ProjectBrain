...
public class OVRManager : MonoBehaviour
{
	...
	/// </summary>
	[Tooltip("참이면 파이프 라인의 실속을 피하고 CPU-GPU 병렬 처리를 증가시키기 위해 왜곡 렌더링 작업이 일찍 쿼터 프레임에 제출됩니다.If true, distortion rendering work is submitted a quarter-frame early to avoid pipeline stalls and increase CPU-GPU parallelism.")]
	...
	/// </summary>
	[Tooltip("true이면 Unity는 현재 하드웨어의 품질 / 성능에 최적의 앤티 엘리 어싱 레벨을 사용합니다. If true, Unity will use the optimal antialiasing level for quality/performance on the current hardware.")]
	public bool useRecommendedMSAALevel = false;

	/// <summary>
	/// If true, dynamic resolution will be enabled
	/// </summary>
	[Tooltip("참이면 동적 해상도가 PC에서 활성화됩니다. If true, dynamic resolution will be enabled On PC")]
	...
	[Tooltip("앱이 적응 해상도 모드로 도달 할 수있는 최소 렌더링스케일 Min RenderScale the app can reach under adaptive resolution mode")]
	...
	/// </summary>
	[RangeAttribute(0.5f, 2.0f)]
	[Tooltip("앱이 적응 해상도 모드로 도달 할 수있는 최대 렌더링스케일. Max RenderScale the app can reach under adaptive resolution mode")]
	...
	[Header("Tracking")]
	[SerializeField]
	[Tooltip("현재 추적 원점 유형을 정의합니다. Defines the current tracking origin type.")]
	...
	/// </summary>
	[Tooltip("true이면 머리 추적은 각 OVRCameraRig의 카메라 위치에 영향을 미칩니다. If true, head tracking will affect the position of each OVRCameraRig's cameras.")]
	...
	/// </summary>
	[Tooltip("true 인 경우, 사용자의 눈 사이의 거리가 각 OVRCameraRig의 카메라 위치에 영향을 미칩니다. If true, the distance between the user's eyes will affect the position of each OVRCameraRig's cameras.")]
	public bool useIPDInPositionTracking = true;

	/// <summary>
	/// If true, each scene load will cause the head pose to reset.
	/// </summary>
	[Tooltip("true이면 각 장면로드로 인해 머리 포즈가 재설정됩니다. If true, each scene load will cause the head pose to reset.")]
    ...
    /// 참이면 유니버설 메뉴의 Reset View가 포즈를 재설정합니다. 이것은 일반적으로 가상 세계에서 고정 된 위치의 응용 프로그램에 대해 
    /// 활성화되어야하며보기 다시 설정 명령을 통해 해당 사람을 미리 정의 된 위치 (예 : 콕핏 좌석)로 되돌릴 수 있습니다. 
    /// 뷰를 재설정하면 잠재적으로 유효하지 않은 위치로 플레이어를 텔레포트 할 수 있기 때문에 이동 시스템을 사용하는 경우이 값을 false로 설정하십시오.
    /// </summary>
    /// 툴팁 : 참이면 유니버설 메뉴의 Reset View가 포즈를 재설정합니다. 이것은 일반적으로 가상 세계에서 고정 된 위치의 응용 프로그램에 대해 활성화되어야하며보기 다시 설정 명령을 통해 해당 사람을 미리 정의 된 위치 (예 : 콕핏 좌석)로 되돌릴 수 있습니다. 뷰를 재설정하면 잠재적으로 유효하지 않은 위치로 플레이어를 텔레포트 할 수 있기 때문에 이동 시스템을 사용하는 경우이 값을 false로 설정하십시오.
    [Tooltip("참이면 유니버설 메뉴의 Reset View가 포즈를 재설정합니다. 이것은 일반적으로 가상 세계에서 고정 된 위치의 응용 프로그램에 대해 활성화되어야하며보기 다시 설정 명령을 통해 해당 사람을 미리 정의 된 위치 (예 : 콕핏 좌석)로 되돌릴 수 있습니다. 뷰를 재설정하면 잠재적으로 유효하지 않은 위치로 플레이어를 텔레포트 할 수 있기 때문에 이동 시스템을 사용하는 경우이 값을 false로 설정하십시오. If true, the Reset View in the universal menu will cause the pose to be reset. This should generally be enabled for applications with a stationary position in the virtual world and will allow the View Reset command to place the person back to a predefined location (such as a cockpit seat). Set this to false if you have a locomotion system because resetting the view would effectively teleport the player to potentially invalid locations.")]
